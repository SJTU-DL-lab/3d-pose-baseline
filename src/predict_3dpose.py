...
train_dir = FLAGS.train_dir#删完之后就能在windows系统上用了  #FLAGS.train_dir的地址即experiment

print( train_dir )
summaries_dir = os.path.join( train_dir, "log" ) # Directory for TB summaries TB就是TensorBord   experiment.log就是TB的目录

# To avoid race conditions: https://github.com/tensorflow/tensorflow/issues/7448
os.system('mkdir -p {}'.format(summaries_dir))#即在{}中填上TB的地址，即生成这个子目录

def create_model( session, actions, batch_size ):#主要分为两部分：要么生成模型，要么加载之前的checkpoint
  """
  ...
  if FLAGS.load <= 0:
    ...
    session.run( tf.global_variables_initializer() )#session.run使用已定义好的会话来计算关心的结果  在TensorFlow中一个变量的值在被明确调用前，这个变量的初始化过程需要被明确地调用。
    return model

  # Load a previously saved model
  ckpt = tf.train.get_checkpoint_state( train_dir, latest_filename="checkpoint") #experiment中的checkpoint
...
##########################下面都是用来checkpoint的#############################
  if ckpt and ckpt.model_checkpoint_path:
    ...
    if FLAGS.load > 0:
      if os.path.isfile(os.path.join(train_dir,"checkpoint-{0}.index".format(FLAGS.load))):#train_dir+'checkpoint-1.index' 即experiment.checkpoint-24371.index是否为文件
        ckpt_name = os.path.join( os.path.join(train_dir,"checkpoint-{0}".format(FLAGS.load)) ) # experiments\checkpoint-24371
      else:
        raise ValueError("Asked to load checkpoint {0}, but it does not seem to exist".format(FLAGS.load))
    else:
      ckpt_name = os.path.basename( ckpt.model_checkpoint_path ) #返回path最后的文件名

    print("Loading model {0}".format( ckpt_name ))
    model.saver.restore( session, ckpt.model_checkpoint_path )  #之前保存的结果会被加载到当前的对话中
...
def train():
  ...
  actions = data_utils.define_actions( FLAGS.action )  #returns a list of corresponding actions

  number_of_actions = len( actions ) 

  # Load camera parameters
  SUBJECT_IDS = [1,5,6,7,8,9,11]  #1,5,6,7,8 for train, 9,11 for test
  rcams = cameras.load_cameras(FLAGS.cameras_path, SUBJECT_IDS) #得到了关于camera的参数
  #print("相机的参数")
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    # === Create the model ===
    print("Creating %d bi-layers of %d units." % (FLAGS.num_layers, FLAGS.linear_size))#打印出来的是Creating 2 bi-layers of 1024 units.
    model = create_model( sess, actions, FLAGS.batch_size )#FLAGS的batch_size是64
    ...
    for _ in xrange( FLAGS.epochs ):  #与range类似但也有不同之处
      ...
      print("There are {0} train batches".format( nbatches )) #24371个branches
      ...
      if FLAGS.evaluateActionWise:
        ...
        print("{0:=^12} {1:=^6}".format("Action", "mm")) # line of 30 equal signs  即为====Action====mm
...
def get_action_subset( poses_set, action ):
  ...
  Returns
    poses_subset: dictionary with same structure as poses_set, but only with the
      specified action.
  """
  #因为K=(subject,action,seqname)所以需要K[1]==action才可
...
def evaluate_batches( sess, model,
  ...
  for i in range(nbatches):
    ...
    enc_in, dec_out = encoder_inputs[i], decoder_outputs[i]  #64*32 64*48
...
def sample():
  ...
  print (enc_in.shape,"enc_in的大小为多少",dec_out.shape,poses3d.shape)
