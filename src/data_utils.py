...
def load_data( bpath, subjects, actions, dim=3 ):
  ...
  for subj in subjects:
    for action in actions:
      ...
      dpath = os.path.join( bpath, 'S{0}'.format(subj), 'MyPoses/{0}D_positions'.format(dim), '{0}*.h5'.format(action) )#例如进入到了S1的
      print( dpath )

      fnames = glob.glob( dpath )    #glob用于文件名模式匹配  到底有啥用？  每个action都有2个h5文件 但某些会因为action名字相似而有4个，下一步需要去剔除

      loaded_seqs = 0
      for fname in fnames:
        seqname = os.path.basename( fname ) #返回path最后的文件名

        # This rule makes sure SittingDown is not loaded when Sitting is requested 这里所做的应该就是防止因为文件名而读取错误
        ...
        if seqname.startswith( action ):
          ...
          with h5py.File( fname, 'r' ) as h5f:
            poses = h5f['{0}D_positions'.format(dim)][:]  #pose的维度为（96，n)不同h5文件下的n都不一样
            #print(poses,poses.shape,"poses是个啥")

          poses = poses.T  #现在转换为了（n,96)的大小了
...
def load_stacked_hourglass(data_dir, subjects, actions):
  ...
  SH_TO_GT_PERM = np.array([SH_NAMES.index( h ) for h in H36M_NAMES if h != '' and h in SH_NAMES])#将SH中和H36M相匹配的点排列起来
  assert np.all( SH_TO_GT_PERM == np.array([6,2,1,0,3,4,5,7,8,9,13,14,15,12,11,10]) )#np.all 用来比较两者是否相等时很有用,将sh和h3.6m的数据对应起来
    ...
    for action in actions:
      ...
      for fname in fnames:
        ...
        if seqname.startswith( action ):
          ...
          with h5py.File( fname, 'r' ) as h5f:
            poses = h5f['poses'][:] #(1612*16*2)

            # Permute the loaded data to make it compatible with H36M,让sh的数据格式与H36M的匹配起来
            poses = poses[:,SH_TO_GT_PERM,:]

            # Reshape into n x (32*2) matrix
            poses = np.reshape(poses,[poses.shape[0], -1])#从1612*16*2再转为1612*32的格式了
            ...
            seqname = seqname+'-sh'
            data[ (subj, action, seqname) ] = poses_final #只是将16*2的数据填入到32*2当中
            #print("2d数据的维度",poses_final.shape)  n*64
...
def normalization_stats(complete_data, dim, predict_14=False ):
  ...
  data_mean = np.mean(complete_data, axis=0)  #(1*96)or(1*64)
  data_std  =  np.std(complete_data, axis=0)  #(1*96)
  ...
  if dim == 2:
    ...
    dimensions_to_use = np.where(np.array([x != '' for x in H36M_NAMES]))[0] #得到的是H3.6M中用的到的索引号
   # print("dimensions_to_use",dimensions_to_use)        np.where 满足条件输出
    
    dimensions_to_use = np.delete( dimensions_to_use, [0,7,9] if predict_14 else 0 )

    dimensions_to_use = np.sort( np.hstack( (dimensions_to_use*3,      #将32*3个点中有用的都挑出来
                                             dimensions_to_use*3+1,
                                             dimensions_to_use*3+2)))
    dimensions_to_ignore = np.delete( np.arange(len(H36M_NAMES)*3), dimensions_to_use ) #范围是0-95这96个点
...
def transform_world_to_camera(poses_set, cams, ncams=4 ):
    ...
    """
    t3d_camera = {}
    for t3dk in sorted( poses_set.keys() ):#获取已排序的列表副本，本例中即为键按照某种顺序排序后的列表：好像是字母顺序排列的
      ...
      for c in range( ncams ):
        ...
        camera_coord = cameras.world_to_camera_frame( np.reshape(t3d_world, [-1, 3]), R, T) #np.reshape()将其变为(m,3)
        #print("原camera_coord的大小",camera_coord.shape)
        camera_coord = np.reshape( camera_coord, [-1, len(H36M_NAMES)*3] )  #(n,96) 96是因为h3.6m一共有32个点，32*3=96
...
def unNormalizeData(normalized_data, data_mean, data_std, dimensions_to_ignore):
  ...
  """
  T = normalized_data.shape[0] # Batch size 这里就是64
  D = data_mean.shape[0] # Dimensionality 96or64

  orig_data = np.zeros((T, D), dtype=np.float32)
  dimensions_to_use = np.array([dim for dim in range(D)
                                if dim not in dimensions_to_ignore])#得到一个array中放着要用的dimension的index号
...
def read_2d_predictions( actions, data_dir ):
  ...
  complete_train = copy.deepcopy( np.vstack( train_set.values() ))  #应该是N*64
  data_mean, data_std,  dim_to_ignore, dim_to_use = normalization_stats( complete_train, dim=2 )#就求了一个均值和方差，dim-to-ignore和dim-to-use之前已经求过来
...
def read_3d_data( actions, data_dir, camera_frame, rcams, predict_14=False ):
  ...
  """
  # Load 3d data
  train_set = load_data( data_dir, TRAIN_SUBJECTS, actions, dim=3 )  #TRAIN_SUBJECTS=[1,5,6,7,8] train_set即data[subject,action,ncam]:
  #print(train_set,"train_set 是个啥") #train_set.shape是个dict
  test_set  = load_data( data_dir, TEST_SUBJECTS,  actions, dim=3 )

  if camera_frame:  #本案例中为true 数据扩大了四倍，因为有四个摄像头
    train_set = transform_world_to_camera( train_set, rcams )
    test_set  = transform_world_to_camera( test_set, rcams )

  # Apply 3d post-processing (centering around root) 运用3D的post-processing 将坐标转化为围绕center的
  train_set, train_root_positions = postprocess_3d( train_set )
  test_set,  test_root_positions  = postprocess_3d( test_set )

  # Compute normalization statistics
  complete_train = copy.deepcopy( np.vstack( train_set.values() ))#.values方法返回的是一个由字典中的值组成的字典视图。
  #print("complete_train的维度",complete_train.shape)  堆叠起来就是一个（N，96） 堆叠起来只是为了求其mean和std，再将其还原回去
...
def postprocess_3d( poses_set ):
  ...
  for k in poses_set.keys():
    ...
    root_positions[k] = copy.deepcopy(poses_set[k][:,:3])  #shallow copy and deep copy的区别
    #print("每个root的大小",root_positions[k].shape) #(n,3)的大小——只是第一个root点的坐标被记录下来了
    # Remove the root from the 3d position
    poses = poses_set[k]
    poses = poses - np.tile( poses[:,:3], [1, len(H36M_NAMES)] ) #tile函数将举证横向或者纵向复制，即以根节点为原点
    poses_set[k] = poses
    #print("每个pose的大小",poses_set[k].shape) #(n,96)的大小
