...
def load_cameras( bpath='cameras.h5', subjects=[1,5,6,7,8,9,11] ):
  ...
  with h5py.File(bpath,'r') as hf:  #打开一个HDF5文件 分为/3dtest /subject1 .../subject11 其中每个subject是一个group
    for s in subjects:
      for c in range(4): # There are 4 cameras in human3.6m
        rcams[(s, c+1)] = load_camera_params(hf, 'subject%d/camera%d/{0}' % (s,c+1) ) #subject s/camera c+1{0}  'subject1/camera3/T'
